...
RSpec.describe GramsController, type: :controller do
	...
	describe "grams#destroy action" do
		...
		it "should return a 404 message if we cannot find a gram with the id that is specified" do
			user = FactoryBot.create(:user)
			sign_in user
			delete :destroy, params: {id: "SPACEFLUNKY"}
	...
	describe "grams#update action" do
		it "shouldn't let users who didn't create the gram update it" do
			...
			user = FactoryBot.create(:user)
			sign_in user
			patch :update, params: { id: gram.id, gram: { message: 'Eekum-bokum!' } }
			expect(response).to have_http_status(:forbidden)
		end
		...
		it "should have http 404 error if the gram cannot be found" do
			...
			patch :update, params: { id: "NINJABEAM", gram: { message: 'Changed Value' } }
			expect(response).to have_http_status(:not_found)
		end
	...
	describe "grams#edit action" do
		...
		it "should return a 404 error message if the gram is not found" do
			...
			get :edit, params: { id: 'GatitoDelTaquito' }
			expect(response).to have_http_status(:not_found)
		end
	end
	...
	describe "grams#show action" do
		...
		it "should return a 404 error if the gram is not found" do
			get :show, params: { id: 'GatoDelTaco' }
			expect(response).to have_http_status(:not_found)
		end
	end
